...
ANALYSIS_SKILL_RELEASE_ZIP = f"foggy-ai-analysis-skill-{ANALYSIS_SKILL_RELEASE_VERSION}.zip"
...
ANALYSIS_SKILL_ZIP_INSTALL_COMMAND = (
    f"foggy-runtime skills install {ANALYSIS_SKILL} --zip {ANALYSIS_SKILL_RELEASE_ZIP} --replace"
)
ANALYSIS_SKILL_WORKSPACE_INSTALL_COMMAND = (
    f"foggy-runtime skills install {ANALYSIS_SKILL} --workspace-root <workspace-root> --replace"
)
...
SKILL_HELP = {
    ANALYSIS_SKILL: "onboarding, runtime setup, semantic modeling, and bundled sales-drop demo assets",
    SEMANTIC_QUERY_SKILL: "queryModel DSL, Compose/CTE, and governed semantic query workflows",
    "foggy-analysis-suite": "installs both foggy-ai-analysis and foggy-semantic-query",
}


class FoggyArgumentParser(argparse.ArgumentParser):
...
def build_parser() -> argparse.ArgumentParser:
    parser = FoggyArgumentParser(
        prog="foggy-runtime",
        description=(
            "Foggy Runtime API v1 CLI. Use 'skills install foggy-ai-analysis' for onboarding "
            "and bundled sales-drop demo assets, 'stack show' to inspect recommended releases, "
            "and 'demo sales-drop' to replay the demo."
        ),
    )
    ...
    stack_show.add_argument(
        "--stack-manifest",
        help=f"Path or URL to a foggy-stack/v1 manifest. Defaults to {DEFAULT_STACK_MANIFEST_URL}.",
    )
    stack_show.add_argument(
        "--offline-stack",
        action="store_true",
    ...
    datasource_add.add_argument(
        "--type",
        default="sqlite",
        help="Runtime datasource type. Defaults to sqlite; other JDBC-backed types are validated by the runtime.",
    )
    ...
    datasource_add_password.add_argument(
        "--password-env",
        help="Environment variable name containing the password; sent as passwordRef=env:<name>.",
    )
    ...
    datasource_update_password.add_argument(
        "--password-env",
        help="Environment variable name containing the password; sent as passwordRef=env:<name>.",
    )
    ...
    datasource_diagnostics = datasource_commands.add_parser("diagnostics")
    datasource_diagnostics.set_defaults(
        method="GET",
        path="/api/v1/datasources/diagnostics",
        body_builder=no_body,
        required_capabilities=["datasources.diagnostics"],
    )
    ...
    skills = subparsers.add_parser(
        "skills",
        help="Install Foggy Skills such as foggy-ai-analysis and foggy-semantic-query.",
    ...
    skill_install = skill_commands.add_parser(
        ...
        epilog=(
            "Skills:\n"
            "  foggy-ai-analysis     Onboarding, runtime setup, semantic modeling, and sales-drop demo assets.\n"
            "  foggy-semantic-query  queryModel DSL, Compose/CTE, and governed semantic query workflows.\n"
    ...
    skill_source.add_argument("--source-dir", help="Path to an unpacked Skill source directory.")
    skill_source.add_argument("--zip", dest="zip_path", help="Path to a released Skill zip.")
    skill_install.add_argument(
        "--workspace-root",
        help="Workspace root used to auto-discover local Skill sources when --source-dir/--zip is omitted.",
    )
    skill_install.add_argument(
        "--stack-manifest",
        help=f"Path or URL to a foggy-stack/v1 manifest for default release downloads. Defaults to {DEFAULT_STACK_MANIFEST_URL}.",
    )
    skill_install.add_argument(
        "--offline-stack",
        action="store_true",
        help="Use the CLI built-in fallback stack manifest for default release downloads.",
    )
    skill_install.add_argument(
        "--replace",
        action="store_true",
    ...
    demo = subparsers.add_parser(
        "demo",
        help="Run bundled demos such as sales-drop from foggy-ai-analysis.",
        description="Run Foggy demo helpers. The sales-drop demo uses assets bundled with foggy-ai-analysis.",
    )
    demo_commands = demo.add_subparsers(dest="demo_command", required=True)

    sales_drop = demo_commands.add_parser(
        "sales-drop",
        description=(
            "Plan or replay the bundled sales-drop demo from the foggy-ai-analysis Skill. "
            f"Install it from the release zip with: {ANALYSIS_SKILL_ZIP_INSTALL_COMMAND}"
        ),
        epilog=(
            "Recommended Skill dir: ~/.agents/skills/foggy-ai-analysis\n"
            f"Release zip: {ANALYSIS_SKILL_RELEASE_ZIP_URL}\n"
            "Legacy foggy-ai-analysis-demo assets are accepted only as a compatibility fallback."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    sales_drop_commands = sales_drop.add_subparsers(dest="sales_drop_command", required=True)

    sales_drop_plan = sales_drop_commands.add_parser(
        "plan",
        description=(
            "Emit a local command plan for the sales-drop demo. Defaults to the installed "
            "foggy-ai-analysis Skill under ~/.agents/skills when available."
        ),
    )
    ...
    sales_drop_plan.add_argument(
        "--skill-dir",
        help=(
            "Path to an unpacked foggy-ai-analysis Skill directory. "
            "Defaults to ~/.agents/skills/foggy-ai-analysis, then workspace development sources."
        ),
    )
    sales_drop_plan.add_argument(
        "--port",
    ...
    sales_drop_replay = sales_drop_commands.add_parser(
        "replay",
        description=(
            "Replay the sales-drop demo against a running Runtime API using foggy-ai-analysis assets. "
            "If --skill-dir is omitted, ~/.agents/skills/foggy-ai-analysis is used when present."
        ),
    )
    sales_drop_replay.add_argument(
        "--skill-dir",
        help="Path to an unpacked foggy-ai-analysis Skill directory. Defaults to ~/.agents/skills/foggy-ai-analysis.",
    )
    sales_drop_replay.add_argument(
        "--evidence-dir",
    ...
    sales_drop_replay.add_argument(
        ...
        help=(
            ...
            "Use this for the bundled sales-drop SQLite replay so demo seeding never touches a user datasource."
        ),
    )
...
def resolve_base_url(args: argparse.Namespace) -> str:
    ...
    if getattr(args, "api_domain", "runtime") == "analytics":
        return analytics_cli.resolve_base_url(args)
...
def resolve_auth_code(args: argparse.Namespace) -> str | None:
    ...
    if getattr(args, "api_domain", "runtime") == "analytics":
        return analytics_cli.resolve_auth_code(args)
...
def resolve_authorization(args: argparse.Namespace) -> str | None:
    ...
    if getattr(args, "api_domain", "runtime") == "analytics":
        return analytics_cli.resolve_authorization(args)
...
def datasource_add_body(args: argparse.Namespace, _stdin: TextIO) -> dict[str, Any]:
    ...
    password_ref = datasource_password_ref(args)
    if password_ref:
        body["passwordRef"] = password_ref
    return body
...
def datasource_password_ref(args: argparse.Namespace) -> str | None:
    password_env = getattr(args, "password_env", None)
    if password_env:
        return f"env:{password_env}"
    return getattr(args, "password_ref", None)
    ...
    try:
        if requested_skill in SKILL_SUITES:
            ...
            if not args.workspace_root:
                stack_manifest, stack_source, stack_warnings = load_stack_manifest(
                    args,
        ...
        if args.zip_path:
            ...
            with tempfile.TemporaryDirectory(prefix="foggy-skill-install-") as temp_dir:
                ...
                return response
        if not args.workspace_root:
            stack_manifest, stack_source, stack_warnings = load_stack_manifest(args, allow_default_fallback=False)
...
def prepare_skill_from_release(
    ...
    spec = stack_skill_spec(stack_manifest, skill_name)
    ensure_skill_cli_compatible(stack_manifest, skill_name, spec)
    zip_spec = spec["zip"]
    zip_url = str(zip_spec["url"])
    zip_name = safe_release_zip_name(zip_spec, zip_url, skill_name)
...
def demo_sales_drop_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    skill_dir, candidate_skill_dirs = resolve_sales_drop_skill_dir(args.skill_dir, repo_root)
    demo_dir = sales_drop_demo_dir(skill_dir)
    ...
    sqlite_path = (
        ...
        if args.sqlite_path
        else repo_root / ".codex-tmp" / "foggy-ai-analysis" / "sales-drop" / "foggy_mcp_lite.db"
    )
    launcher_jar = (
    ...
    if missing_assets:
        return {
            ...
            "data": {
                ...
                "candidateSkillDirs": [str(path) for path in candidate_skill_dirs],
                "installCommand": ANALYSIS_SKILL_ZIP_INSTALL_COMMAND,
                "publicZipUrl": ANALYSIS_SKILL_RELEASE_ZIP_URL,
                "zipInstallCommand": ANALYSIS_SKILL_ZIP_INSTALL_COMMAND,
                "workspaceInstallCommand": ANALYSIS_SKILL_WORKSPACE_INSTALL_COMMAND,
            },
            "error": {
                "code": "DEMO_ASSET_MISSING",
                "phase": "demo.sales-drop.plan",
                "message": (
                    "Required sales-drop demo assets are missing. Download the foggy-ai-analysis "
                    f"release zip from {ANALYSIS_SKILL_RELEASE_ZIP_URL} and install it with "
                    f"'{ANALYSIS_SKILL_ZIP_INSTALL_COMMAND}', install from a workspace with "
                    f"'{ANALYSIS_SKILL_WORKSPACE_INSTALL_COMMAND}', or pass --skill-dir to an "
                    "unpacked foggy-ai-analysis Skill."
                ),
                "safeToAutoRepair": False,
            },
        }
...
def demo_sales_drop_replay(
    ...
    skill_dir, candidate_skill_dirs = resolve_sales_drop_skill_dir(args.skill_dir)
    demo_dir = sales_drop_demo_dir(skill_dir)
    schema = demo_dir / "schema.sql"
    data = demo_dir / "data.sql"
    models_dir = Path(args.models_dir).resolve() if args.models_dir else demo_dir / "models"
    ...
    summary = {
        ...
        "results": results,
        "notes": [
            "This command uses an unpacked foggy-ai-analysis Skill directory and a running Runtime API.",
...
def demo_write_replay_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Scope",
            "",
            "This replay uses a running Runtime API plus an unpacked foggy-ai-analysis Skill directory. "
            "Production permission, auth, RBAC, audit, and governance remain deferred.",
            "",
        ]
    )
